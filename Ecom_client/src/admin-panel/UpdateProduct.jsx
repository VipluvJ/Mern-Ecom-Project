...
//   { key: "Mens Apparrel", value: "Mens Apparrel" },
//   { key: "Womens Apparrel", value: "Womens Apparrel" },
//   { key: "Kids Apparrel", value: "Kids Apparrel" },
// ];
//-------------------------
const UpdateProduct = () => {
  ...
  const { listOfCategory } = useSelector((state) => state.allCategory);
  const idObj = useParams();
  const productDetails = listOfProducts.filter((data) => {
    return data._id === idObj.id;
  });
  const defaultValue = listOfCategory.find((selected) => {
    return selected._id === productDetails[0].category;
  });

  console.log(defaultValue);
  //   Not Working Select -----------------------
  //   const defaultValue = categoryOptions.findIndex((val) => {
  //     let index;

  //     for (let i = 0; i <= listOfCategory.length; i++) {
  //       console.log(listOfCategory);
  //       let { name } = listOfCategory[i];
  //       if (name === val.key.toLowerCase()) {
  //         index = i;
  //         console.log(index);
  //       }
  //     }
  ...
  const defaultValues = {
    ...
    deliveryInformation: productDetails[0].deliveryInformation,
